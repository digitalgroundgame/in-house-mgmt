...
class TestCanCommentOnTicketPermission:
    ...
    def test_denies_unauthenticated(self, rf, assigned_ticket):
        ...
        assert not self.perm.has_object_permission(request, self.view, assigned_ticket)

    def test_allows_superuser(self, rf, admin_user, unassigned_ticket):
        request = rf.post("/")
        request.user = admin_user

        assert self.perm.has_object_permission(request, self.view, unassigned_ticket)
    ...
    def test_allows_assigned_user_with_comment_perm(
        self, rf, regular_user, assigned_ticket, add_ticketcomment_permission
    ):
        ...
        assert self.perm.has_object_permission(request, self.view, assigned_ticket)
    ...
    def test_denies_unassigned_user_with_comment_perm(
        ...
        assert not self.perm.has_object_permission(request, self.view, unassigned_ticket)
...
class TestChangeTicketStatusPermission:
    ...
    def test_change_status_global_permission(
